...
Then copy over the UEFI/EDK2 files that were built in the previous steps, copying them to the right firmware folder location:
...
Now just copy cover all the files that are now available at the @vexpress-firmware@ folder into the mounted MMC card (which is provided as an external storage after calling @usb_on@):
