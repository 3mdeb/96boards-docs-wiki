...
Now just flash the firmware that was built on the previous steps:

bc. dpcmd --type MX25L12835F -p FIRMWARE.rom

Now just plug the power cable back and it should boot the new firmware.

h5. Upgrading firmware

There is currently no easy way to update just the UEFI/EDK2 firmware, so please follow the clean flash process as described above.
