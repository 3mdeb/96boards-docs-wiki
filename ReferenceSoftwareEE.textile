...
The reference kernel used by the enterprise release (and soon to be shared with CE) can be found at "https://git.linaro.org/people/amit.kucheria/kernel.git/shortlog/refs/heads/96b-kernelci":https://git.linaro.org/people/amit.kucheria/kernel.git/shortlog/refs/heads/96b-kernelci

Since we want the same kernel config to be used for all our builds and distributions, it is also available as part of the same kernel tree, and can be found at "arch/arm64/configs/distro.config":https://git.linaro.org/people/amit.kucheria/kernel.git/blob/refs/heads/96b-kernelci:/arch/arm64/configs/distro.config

At the time of the 15.12 release, the kernel is based on *4.4-rc4*.

For future releases we will also have kernel config fragments, which will make it easier for other projects and distributions to consume.
