...
Downloading the CentOS *upstream* kernel and initrd:
...
If you want to use the RP development image (which uses the 4.4 RP kernel), please download the following files instead:
...
menuentry 'Install CentOS 7 ARM 64-bit - RP (experimental)' --class red --class gnu-linux --class gnu --class os {
...
bc. $ cat /srv/tftp/grub.cfg 
menuentry 'Install CentOS 7 ARM 64-bit' --class red --class gnu-linux --class gnu --class os {
    linux (tftp)/centos7/vmlinuz ip=eth0:dhcp inst.repo=http://mirror.centos.org/altarch/7/os/aarch64/ inst.ks=http://people.linaro.org/~ricardo.salveti/centos-ks.cfg
    initrd (tftp)/centos7/initrd.img
}
